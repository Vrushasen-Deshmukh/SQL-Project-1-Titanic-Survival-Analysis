...
-- 1. Calculate the percentage of survivors based on embarkation points:
SELECT embarked, COUNT(*) as total_passengers,
	round(((SUM(survived) / COUNT(*))*100),3) as survival_rate
FROM titanic
GROUP BY embarked;

-- 2. Find the total number of passengers grouped by both gender and class:
SELECT sex, p_class, COUNT(*) AS passenger_count
FROM titanic
GROUP BY sex, p_class;

-- 3. Find the distribution of fare amounts per class(min, max and avg)
SELECT p_class, round(MAX(fare),3) AS max_fare,
round(MIN(fare),3) AS min_fare,
round(AVG(fare),3) AS avg_fare
FROM titanic
GROUP BY p_class;


-- USING WINDOWS FUNCTIONS:
-- 1. Calculate the running total of fares paid, ordered by passenger ID:
